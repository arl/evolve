...
func TestSigmaScalingNoVariance(t *testing.T) {
	...
	steve, _ := framework.NewEvaluatedCandidate("Steve", 4.0)
	john, _ := framework.NewEvaluatedCandidate("John", 4.0)
	mary, _ := framework.NewEvaluatedCandidate("Mary", 4.0)
	gary, _ := framework.NewEvaluatedCandidate("Gary", 4.0)
	...
	assert.True(t, steveCount == 1, "candidate selected wrong number of times (should be 1, was ", steveCount, ")")
	assert.True(t, johnCount == 1, "candidate selected wrong number of times (should be 1, was ", johnCount, ")")
	assert.True(t, maryCount == 1, "candidate selected wrong number of times (should be 1, was ", maryCount, ")")
	assert.True(t, garyCount == 1, "candidate selected wrong number of times (should be 1, was ", garyCount, ")")
}
