...
func TestStochasticUniversalSamplingNaturalFitness(t *testing.T) {
	...
	assert.True(t, steveCount >= 2 && steveCount <= 3, "candidate selected wrong number of times (should be 2 or 3, was ", steveCount, ")")
...
func TestStochasticUniversalSamplingNonNaturalFitness(t *testing.T) {
	rng := rand.New(rand.NewSource(99))
	selector := StochasticUniversalSampling{}
	population := make(framework.EvaluatedPopulation, 4)
	...
	assert.True(t, garyCount >= 2 && garyCount <= 3, "candidate selected wrong number of times (should be 2 or 3, was ", garyCount, ")")
