...
// FitnessCache provides caching for any Evaluator implementation. The results
// of fitness evaluations are stored in a cache so that if the same candidate is
// evaluated twice, the expense of the fitness calculation can be avoided the
// second time.
//
// Caching of fitness values can be a useful optimisation in situations where
// the fitness evaluation is expensive and there is a possibility that some
// candidates will survive from generation to generation unmodified.  Programs
// that use elitism are one example of candidates surviving unmodified.  Another
// scenario is when the configured evolutionary operator does not always modify
// every candidate in the population for every generation.
//
