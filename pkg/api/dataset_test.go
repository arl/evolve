...
func TestDataSetAggregate(t *testing.T) {
	data := NewDataSet(WithPrePopulatedDataSet(testDataSet))
...
func TestDataSetMinimum(t *testing.T) {
	data := NewDataSet()
	data.AddValue(4)
	assert.Equal(t, data.Minimum(), 4.0)
	data.AddValue(7)
	assert.Equal(t, data.Minimum(), 4.0)
	data.AddValue(2)
	assert.Equal(t, data.Minimum(), 2.0)
	data.AddValue(-9)
	assert.Equal(t, data.Minimum(), -9.0)
}

func TestDataSetMaximum(t *testing.T) {
	data := NewDataSet()
	data.AddValue(9)
	assert.Equal(t, data.Maximum(), 9.0)
	data.AddValue(8)
	assert.Equal(t, data.Maximum(), 9.0)
	data.AddValue(-15)
	assert.Equal(t, data.Maximum(), 9.0)
	data.AddValue(12)
	assert.Equal(t, data.Maximum(), 12.0)
}
...
func TestDataSetHarmonicMean(t *testing.T) {
	data := NewDataSet(WithPrePopulatedDataSet([]float64{1, 2, 4, 4}))
